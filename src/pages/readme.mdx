...
- 実際にWebブラウザで試せるデモとして、簡易的な[REPL](https://webr.r-wasm.org/latest/)が用意されている
  ...
  - そうなんだけど、そういうものと期待してしまうとけっこう不便
    - 外部のライブラリに依存するRパッケージには動かせないものがある
    - Webページに埋め込んで使う場合、アクセスするたびごとに、使いたいRパッケージをアセットとしてダウンロードしてくる必要がある
...
- たぶん **without the need for an R server to execute the code** の部分が重要
  - Rコードを実行するのにwasmを実行できる環境さえあればOKになった、というのが非常に偉い
...
- 一般的には[shinylive](https://github.com/quarto-ext/shinylive)やQuartoの拡張機能などを利用して、静的なWebページに埋め込んで使う
  ...
  - Quartoの拡張機能としては、コードブロックをインタラクティブなデモにするのに使われる
...
>
> If required, this behaviour can be overridden by self-hosting the contents of a webR release package (or building from source) and providing an alternative base URL for the WebROptions.baseUrl configuration setting.
...
- このイメージにはRはインストールされていないが、Node.jsはインストール済み
  ...
  - がっつりRのコードを書きたいわけではないケースだったら、このイメージの中でWebRを使っちゃえばいいのでは？
